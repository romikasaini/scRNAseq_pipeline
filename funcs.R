...
#Annotation file from sc-type
source(
  "https://raw.githubusercontent.com/IanevskiAleksandr/sc-type/master/R/gene_sets_prepare.R"
)
gs_list = gene_sets_prepare(
  "https://raw.githubusercontent.com/romikasaini/scRNAseq_pipeline/main/ScTypeDB_full.xlsx",
  "Immune system"
)

#cell cycle markers loaded with Seurat
load(
  url(
    "https://raw.githubusercontent.com/romikasaini/scRNAseq_pipeline/main/data/cycle.rda"
  )
)

# Install healthy bonemarrow data for seurat: multimodal reference mapping
InstallData("bmcite")
...
SampleQC = function(seurat_obj) {
  ...
  seurat_obj = subset(seurat_obj, subset = log10GenesPerUMI > 0.8 &
                        nFeature_RNA > 200)
  seurat_obj = subset(seurat_obj, subset = percent.mt < ifelse(
    median(seurat_obj$percent.mt) * 3 < 20,
    median(seurat_obj$percent.mt) * 3,
    20
  ))
  #remove 5% top outliers
  seurat_obj = subset(seurat_obj, subset = nFeature_RNA < sort(seurat_obj$nFeature_RNA)[round(length(seurat_obj$nFeature_RNA) *
                                                                                                0.95)])
  ...
  keep_genes <-
    Matrix::rowSums(nonzero) >= as.numeric(summary(Matrix::rowSums(nonzero))[2])
  ...
  ## Normalization
  seurat_obj <-
    NormalizeData(seurat_obj, verbose = TRUE, scale.factor = 1000000) #To obtain CPM values scale factor 10^6
  seurat_obj <-
...
#These functions create the sample statictics regarding cell cycle percentage, ribosome share (range and percentage), mean mitochondrial percentage and erythrocyte percentage creates density plots for ribosomal, hemoglobin and mitochondrial genes
#localMaxima, CalcPercent, CalcTPshare, PlotTP
...
GetSampleMetrics <-
  function(seurat_obj,
           ...
           save = F,
           plotname = "stats") {
    cycle = round(table(seurat_obj@meta.data[["Phase"]]) / length(seurat_obj@meta.data[["Phase"]]) *
                    100,
                  2)
    ...
    if (plot) {
      if (save) {
        ...
        abline(v = ifelse(median(mt) * 3 < 20, median(seurat_obj$percent.mt) *
                            3, 20))
...
# UMAP_dims: Number of dimesions to use for UMAP, default is 15.
...
clustScore = function(Log2FCdata,
  ...
  gs = lapply(1:length(gs), function(d_) {
    GeneIndToKeep = rownames(Log2FCdata) %in% as.character(gs[[d_]])
    rownames(Log2FCdata)[GeneIndToKeep]
  })
  gs2 = lapply(1:length(gs2), function(d_) {
    GeneIndToKeep = rownames(Log2FCdata) %in% as.character(gs2[[d_]])
    rownames(Log2FCdata)[GeneIndToKeep]
  })
  ...
  es = do.call("rbind", lapply(names(gs), function(gss_) {
    sapply(1:ncol(Z), function(j) {
      gs_z = Z[gs[[gss_]], j]
      gz_2 = Z[gs2[[gss_]], j] * -1
      sum_t1 = (sum(gs_z, na.rm = T) / sqrt(length(gs_z[!is.na(gs_z)])))
      sum_t2 = sum(gz_2, na.rm = T) / sqrt(length(gz_2[!is.na(gz_2)]))
  ...
  es.max <-
    es[!apply(is.na(es) | es == "", 1, all), ] # remove na rows
  
  es.max
}

#This function corrects the accuracy of the cluster annotation based on well-known cell-type specific markers.

choosCl = function(df) {
  res = NULL
  for (i in 1:ncol(df)) {
    x = rownames(df)[df[, i] == max(df[, i], na.rm = T) &
                       !is.na(df[, i])]
    if (length(x) > 1) {
...
# multimodal reference mapping, annotation method provided by Seurat
# Uses pre-annotated healthy bone´marrow data as reference to annotate any other bone marrow dataset
...
Multimodal_UMAP <- function(seurat_obj,
                            ref = bm,
                            plot = F,
                            save = F) {
...
PreciseCluster = function(seurat_obj) {
  ...
  PC <-
    subset(obj,
           idents = grep(
             "T cells|CD4|CD8|NK|DC|Dendritic|B cells",
             levels(obj),
             value = T
           ))
...
# precise: Identify the general category of cell types (e.g T cells) or precise subcategories within those (e.g CD4 Memory T cells). Boolean, default is False.

ClustUMAP = function(seurat_obj,
                     matrix,
                     plot = F,
                     save = F,
                     precise = F) {
...
CalcRawCount <- function(seurat_obj, clustname = "customclassif") {
  ...
  for (j in 1:length(clusts)) {
    x = FetchData(
      ...
      slot = "counts",
      vars = gene_list,
      cells = colnames(seurat_obj)[seurat_obj@meta.data[[clustname]] == clusts[j]]
    ) #Fetch gene expression for cells
    #included in each cluster
    x = data.frame(
      Mean = (apply(x, 2, mean)),
      SD = (apply(x, 2, sd)),
      vars = apply(x, 2, function(y)
        paste0(y, collapse = ";"))
    )
    x$clust = clusts[j]
...
# This function calculates of mean expression per cluster for CPM values
